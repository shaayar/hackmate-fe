...
export default function Login() {
  ...
  const { error, loading, signInWithEmail, signInWithGoogle, signInWithGithub } = useAuthContext()
    ...
    toast({
      ...
      title: (
        <div className="flex items-center">
          <Check className="mr-2 h-4 w-4" /> Login Successful
        </div>
      ),
  ...
  const showErrorToast = (message: string) => {
    toast({
      ...
      title: (
        <div className="flex items-center">
          <AlertCircle className="mr-2 h-4 w-4" /> Login Failed
        </div>
      ),
      description: message,
    })
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (success) {
      ...
      showErrorToast(error || "Failed to sign in with Google")
    }
  }
  ...
  const handleGithubSignIn = async () => {
    ...
    if (success) {
      ...
      showErrorToast(error || "Failed to sign in with GitHub")
    }
  }
    ...
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Card className="w-full max-w-md glass-effect">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-gray-200 text-center">Sign in to your account to continue</CardDescription>
        </CardHeader>
          ...
          <div className="text-sm text-center">
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="link" className="text-primary-foreground hover:text-primary">
                  Forgot password?
                </Button>
              </DialogTrigger>
